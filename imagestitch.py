...
DUPLICATE_CORR_THRESHOLD = 0.985
...
def strip_is_duplicate(last_strip, cur_strip):
    if last_strip is None or cur_strip is None:
        return False
    if last_strip.shape != cur_strip.shape:
        return False
    a = cv2.cvtColor(last_strip, cv2.COLOR_BGR2GRAY).astype(np.float32)
    b = cv2.cvtColor(cur_strip, cv2.COLOR_BGR2GRAY).astype(np.float32)
    a -= np.mean(a)
    b -= np.mean(b)
    den = (np.linalg.norm(a) * np.linalg.norm(b)) + 1e-6
    corr = float(np.sum(a * b) / den)
    return corr >= float(DUPLICATE_CORR_THRESHOLD)
...
def main():
    ...
    prebuffer_frames_used = 0
    last_appended_strip = None
    ...
    while True:
        ...
        if (not capturing) and (vehicle_present or moving):
            ...
            for _, broi, bgray, bsharp in buffered:
                ...
                if strip_is_duplicate(last_appended_strip, strip):
                    frames_skipped_duplicate += 1
                else:
                    pano = append_strip(pano, strip, direction, SEAM_BLEND_PX)
                    strips_added += 1
                    frames_used += 1
                    prebuffer_frames_used += 1
                    strip_sum += strip.shape[1]
                    residual_dx -= np.sign(residual_dx) * step
                    last_appended_strip = strip
                prev_gray = bgray
                prev_roi = broi
            ...
            if prev_gray is None:
                ...
                if is_sharp:
                    ...
                    while abs(residual_dx) >= MIN_STRIP_WIDTH:
                        ...
                        if strip_is_duplicate(last_appended_strip, strip):
                            frames_skipped_duplicate += 1
                            residual_dx -= np.sign(residual_dx) * max(MIN_STRIP_WIDTH, step // 2)
                        else:
                            pano = append_strip(pano, strip, direction, SEAM_BLEND_PX)
                            strips_added += 1
                            frames_used += 1
            ...
            if presence_age >= NO_MOTION_STOP_SEC and motion_age >= NO_MOTION_STOP_SEC:
                ...
                if pano is not None and pano.size > 0:
                    ...
                    print(
                        ...
                        f"frames_used={frames_used} strips_added={strips_added} frames_skipped_duplicate={frames_skipped_duplicate} "
                        f"prebuffer_frames_used={prebuffer_frames_used} avg_strip_w={avg_w:.2f} "
